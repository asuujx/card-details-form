...
function CardForm() {
  ...
  const submitForm = () => {
    console.log(`${formName} + ${formNumber} + ${formDateMonth} + ${formDateYear} + ${formCVC}`);
  };
  ...
  return (
    <div className="max-w-screen px-6 flex flex-col gap-4 mt-28">
      ...
      <div className="flex">
        <FormDate formDateMonth={formDateMonth} setFormDateMonth={setFormDateMonth} formDateYear={formDateYear} setFormDateYear={setFormDateYear} />
        <FormCVC formCVC={formCVC} setFormCVC={setFormCVC} />
      </div>

      <button className="w-full bg-[#220930] text-white py-4 mt-5 rounded-lg" onClick={submitForm}>
        Confirm
      </button>
    </div>
  );
